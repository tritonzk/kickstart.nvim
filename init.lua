...
vim.opt.smartcase = true -- case-sensitive in special cases
...
vim.opt.listchars = { tab = '» ', trail = '·', nbsp = '␣' } -- changes how whitespace chars are displayed
...
-- create keymaps for running python
vim.api.nvim_create_autocmd({ 'FileType' }, {
  pattern = 'python',
  command = "map <buffer> <F6> :w<CR>:exec '!python' shellescape(@%,1)<CR>",
})
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    dependencies = {
      ...
      { -- If encountering errors, see telescope-fzf-native README for installation instructions
        ...
        build = 'make', -- only run when installed/updated
        cond = function() -- condition for installing/loading
    ...
    config = function()
      ...
      local builtin = require 'telescope.builtin' -- NOTE: see :help telescope.builtin
  ...
  { --  NOTE: LSP config and plugins
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', { -- this function gets run when attached to buffer
        ...
        callback = function(event)
          ...
          -- highlight word under cursor. See `:help CursorHold` for information about when this is executed
      ...
      vim.api.nvim_create_autocmd('LspDetach', { -- run when lsp detaches from buffer
